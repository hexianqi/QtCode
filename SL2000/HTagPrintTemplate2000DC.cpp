#include "HTagPrintTemplate2000DC_p.h"
#include "HeCore/HCore.h"
#include <QtGui/QPainter>
#include <QtPrintSupport/QPrinter>

HTagPrintTemplate2000DC::HTagPrintTemplate2000DC(QObject *parent) :
    HAbstractPrintTemplate(*new HTagPrintTemplate2000DCPrivate, parent)
{
}

HTagPrintTemplate2000DC::HTagPrintTemplate2000DC(HTagPrintTemplate2000DCPrivate &p, QObject *parent) :
    HAbstractPrintTemplate(p, parent)
{
}

HTagPrintTemplate2000DC::~HTagPrintTemplate2000DC()
{
}

QString HTagPrintTemplate2000DC::typeName()
{
    return "HTagPrintTemplate2000DC";
}

bool HTagPrintTemplate2000DC::printPages(QPrinter *printer)
{
//   printer->setPageSizeMM(QSize(50, 50));
//    printer->setPageMargins(20, 20, 20, 20, QPrinter::Millimeter);
    QPainter painter(printer);
    int i;
    QVector<double> x,y;
    auto gap = 1.0;
    auto pen = QPen(Qt::black, 0.3);
    auto rect = QRectF(1, 1, 48, 48);
    ...
    painter.setWindow(printer->pageRect(QPrinter::Millimeter).toRect());
    ...
    drawText(&painter, x[0] + gap, y[0], x[2] - gap, y[1], tr("设备型号:SL2000 测试时间:%1").arg(toString("[测量日期时间]")));
    drawText(&painter, x[0] + gap, y[1], x[2] - gap, y[2], tr("产品型号:%1").arg(toString("[产品型号]")));
    drawText(&painter, x[0] + gap, y[2], x[2] - gap, y[3], tr("产品编号:%1").arg(toString("[样品编号]")));
...
void HTagPrintTemplate2000DC::drawText(QPainter *painter, double x1, double y1, double x2, double y2, QString text)
{
