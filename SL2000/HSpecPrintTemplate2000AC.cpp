...
HSpecPrintTemplate2000AC::HSpecPrintTemplate2000AC(HSpecPrintTemplate2000ACPrivate &p, QObject *parent) :
...
{
    return "HSpecPrintTemplate2000AC";
}

void HSpecPrintTemplate2000AC::paintBody(QPainter *painter, QRectF rect, int /*page*/)
{
    ...
    text = tr(" 交流电压：%1V    交流电流：%2A ").arg(toString("[交流电压]"), toString("[交流电流]"));
    painter->drawText(QRectF(x, y , w, h2), Qt::AlignLeft | Qt::AlignVCenter, text);
    y += h2;
    text = tr(" 交流电功率：%1W    功率因数：%2").arg(toString("[交流电功率]"), toString("[功率因数]"));
    painter->drawText(QRectF(x, y , w, h2), Qt::AlignLeft | Qt::AlignVCenter, text);
    y += h2;
    // 颜色参数
    painter->setFont(font1);
...
{
    Q_D(HSpecPrintTemplate2000AC);
    d->types = HCore::membership(QStringList() << "|产品信息2|" << "|环境信息|" << "|时间信息|" << "|光谱信息2|" << "|光度信息2|" << "|光合信息|" << "|色容差信息|" << "|TM30信息|" << "|交流电信息|");
