...
void HIconFontStyle::StyleColor::setColor(const QString &normalBackground, const QString &normalTextColor, const QString &darkBackground, const QString &darkTextColor)
{
...
HIconFontStyle::HIconFontStyle(QObject *parent) :
    QObject(parent),
...
HIconFontStyle::~HIconFontStyle() = default;

QFont HIconFontStyle::getFont()
{
    return d_ptr->font;
}

QPixmap HIconFontStyle::getPixmap(const QColor &color, QChar icon, quint32 fontSize, QSize imageSize, int flags)
{
    d_ptr->font.setPixelSize(fontSize);
    return QPixmap::fromImage(HDrawHelper::createFontImage(d_ptr->font, icon, imageSize, color, flags));
}

void HIconFontStyle::setFamilie(const QString &value)
{
...
void HIconFontStyle::setIcon(QLabel *label, QChar icon, quint32 size)
{
...
void HIconFontStyle::setIcon(QAbstractButton *btn, QChar icon, quint32 size)
{
    d_ptr->font.setPixelSize(size);
...
void HIconFontStyle::setPixmap(QAbstractButton *btn, const QColor &color, QChar icon, quint32 fontSize, QSize imageSize, int flags)
{
    btn->setIcon(getPixmap(color, icon, fontSize, imageSize, flags));
}

void HIconFontStyle::setStyle(QWidget *widget, QList<QAbstractButton *> btns, QList<QChar> icons, const HIconFontStyle::StyleColor &styleColor)
{
    ...
    for (i = 0; i < btns.count(); i++)
    {
        for (j = 0; j < d_ptr->btns.count(); j++)
        {
            ...
            {
                disconnect(btns.at(i), &QAbstractButton::toggled, this, &HIconFontStyle::toggled);
    ...
    for (i = 0; i < btns.count(); i++)
    {
        ...
        connect(btn, &QAbstractButton::toggled, this, &HIconFontStyle::toggled);
...
void HIconFontStyle::init()
{
    d_ptr->factory = new HIconFontFactory(this);
    setFamilie("FontAwesome");
}

void HIconFontStyle::toggled(bool checked)
{
    // 选中和不选中设置不同的图标
...
bool HIconFontStyle::eventFilter(QObject *watched, QEvent *event)
{
