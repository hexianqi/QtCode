#include "HIntegrateSqlPrint_p.h"
...
HIntegrateSqlPrint::HIntegrateSqlPrint(QObject *parent) :
    HSpecSqlPrint(*new HIntegrateSqlPrintPrivate, parent)
{
}

HIntegrateSqlPrint::HIntegrateSqlPrint(HIntegrateSqlPrintPrivate &p, QObject *parent) :
    HSpecSqlPrint(p, parent)
{
}

HIntegrateSqlPrint::~HIntegrateSqlPrint()
{
    qDebug() << __func__;
}

QString HIntegrateSqlPrint::typeName()
{
    return "HIntegrateSqlPrint";
}

QString HIntegrateSqlPrint::textForExcel()
{
    QString text;
...
void HIntegrateSqlPrint::paintPage(QPainter *painter, int page)
{
...
void HIntegrateSqlPrint::paintBody(QPainter *painter, double y1, double y2, int /*page*/)
{
    auto gap = 8.0;
    ...
    painter->drawText(QRectF(x, y , w, h2), Qt::AlignLeft | Qt::AlignVCenter, text);
    y += h2;
    text = tr(" 反向电压：%1V    反向漏流：%2mA").arg(toString("ReverseVoltage"), toString("ReverseCurrent"));
    painter->drawText(QRectF(x, y , w, h2), Qt::AlignLeft | Qt::AlignVCenter, text);
    y += h2;
    // 颜色参数
    painter->setFont(font1);
